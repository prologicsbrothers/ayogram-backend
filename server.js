...
function makeObjectKey({ channelKey, originalName }) {
  ...
  return chat_media/${channelKey}/${Date.now()}_${Math.random().toString(36).slice(2, 8)}_${safeName};
}
...
app.post('/presign-upload', verifyToken, async (req, res) => {
  ...
  if (!fileName || !contentType || !channelKey) return res.status(400).send({ error: 'missing' });
  ...
  // Replace S3 hostname → Public R2.dev URL
